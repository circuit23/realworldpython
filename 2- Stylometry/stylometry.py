...
def find_shortest_corpus(words_by_author):
    """Return the length of shortest corpus."""
    word_count = []
...
def stopwords_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate(words_by_author):
        ...
        stopwords_by_author_freq_dist[author].plot(50, linestyle=LINES[i], label=author, title='50 Most Common '
                                                                                               'Stopwords')

    plt.legend()
    # plt.show(block=True)  # Uncomment to see plot while coding function.


def parts_of_speech_test(words_by_author, len_shortest_corpus):
    """Plot author use of part-of-speech as nouns, verbs, adverbs."""
...
def jaccard_test(words_by_author, len_shortest_corpus):
    ...
    for author in authors:
        ...
        jaccard_sim = (float(len(shared_words)) / (len(unique_words_author) + len(unique_words_unknown) -
                                                   len(shared_words)))
